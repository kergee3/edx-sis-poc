...
`src` アプリは Vercel の専用プロジェクトとして **https://oauth-callback.example.com** に配信している。なお氏名表示に使う IPAmjexMincho Web フォントは別途 https://ipamjexmincho.shumi.dev で外部配信されており（このリポジトリの管理外）、アプリはそれを参照するだけである。
