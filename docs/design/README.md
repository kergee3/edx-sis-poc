...
本プロジェクトが自ら作成する**設計・調査ドキュメント**の置き場所。外部の一次資料（政府PDF/xlsx 等）へのリンク索引は [docs/references/](../references/) にあり、本フォルダはそれらを踏まえて**自分たちで育てていく**文書を置く。

> 区別: `docs/references/` ＝外部出典へのリンク索引（資料ファイルは保持しない）。本フォルダ＝派生的な設計・調査成果物（自由に更新）。
