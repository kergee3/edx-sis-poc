...
> このフォルダは**外部の一次資料へのリンク索引専用**。外部から複製した PDF / xlsx 等の資料ファイルは、このリポジトリには保持しない。これらを踏まえて本プロジェクトが作成する**設計・調査ドキュメントは [docs/design/](../design/) に置く**（混在させない）。
...
- 調査・設計に必要な要約や整理は [docs/design/](../design/) に置き、原典本文の複製にならない範囲に留める。
