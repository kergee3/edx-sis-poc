...
| 生徒一覧 | `/students` | 生徒名簿の表示・編集、転入（追加）／転出（削除）、在学証明書の発行、表示名（姓）の JIS X 0213 マッピング、OneRoster 出力 |
| 設定 | `/settings` | 学校情報、名簿の初期化、表示名編集の JIS X 0213 対応付け候補の生成元、表示名のフォント、Navigation Bar の位置 |
...
- **生徒一覧（`/students`）**: 生徒の一覧を表示する。名簿への追加（転入）・名簿からの削除（転出）ができ、一覧表内で生徒情報を編集できる。さらに**在学証明書の発行**、正式氏名（MJ文字）の「氏（姓）」を **JIS X 0213** の文字へ対応付ける表示名編集、**OneRoster**（名簿連携の国際規格／日本向けの Japan Profile）に沿った出力ができる。
- **設定（`/settings`）**: 学校情報（学校名・校長氏名・学校住所）の編集、名簿の初期化（初期名簿 xlsx の選んだシートで作り直し）、表示名編集の **JIS X 0213 対応付け候補の生成元**（アプリ内蔵の MJ 縮退マップ／[maji.shumi.dev](https://maji.shumi.dev/mj2jis-api) の MJ→JIS 変換 Web API、既定は Web API）、表示名のフォント（ゴシック／明朝）、Navigation Bar の位置を設定する。

### 認証・ユーザーメニュー

Google / LINE に加え、アカウント登録なしですぐ試せる**ゲストログイン**による認証（Auth.js v5）を用いる。ログイン時は**右上に ID が表示**され、それをクリックすると **ログアウト / About / ログイン履歴** などにアクセスできる（ゲストログイン時はバグ報告ボタンを非表示）。ログイン履歴はサーバ／クライアント両側から記録される（`src/features/login-history`）。

### 実装状況

計画していた PoC の機能は一通り実装済み。

- **認証・基盤**: 認証（Google / LINE / ゲスト, Auth.js v5）、ログイン履歴（サーバ＋クライアント）、右上のユーザーメニュー（ログアウト / About / ログイン履歴）、バグ報告（スクショ＋GitHub Issue 起票）。
- **ホーム（`/home`）**: ログイン中の校長氏名・学校名・在籍数を差し込んだ案内表示。
- **生徒一覧（`/students`）**: 名簿の表示・編集、転入（追加）／転出（削除）、在学証明書の発行、表示名（姓）の JIS X 0213 マッピング、OneRoster 出力。
- **設定（`/settings`）**: 学校情報、名簿の初期化、表示名編集の JIS X 0213 対応付け候補の生成元（ローカル／Web API）、表示名のフォント、Navigation Bar の位置。
